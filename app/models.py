...
class Quote(models.Model):
    """
    Цитата с весом, источником и счетчиком просмотров.
    """
    text = models.TextField(unique=True)  # запрет дубликатов
    ...
    def clean(self):
        # лимит: не более 3 цитат на один источник
    ...
    def save(self, *args, **kwargs):
        # атомарная валидация и сохранение (уменьшает риск гонок)
...
class Vote(models.Model):
    ...
    class Meta:
        unique_together = ('user', 'quote')  # по одному голосу на цитату
        indexes = [models.Index(fields=['quote']), models.Index(fields=['user'])]
